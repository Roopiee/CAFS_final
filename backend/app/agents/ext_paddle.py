"""
ext.py
Enhanced extraction agent with triple OCR cascade (Tesseract → PaddleOCR → EasyOCR)
"""
...
class ExtractionAgent:
    """
    Enhanced extraction agent with triple OCR cascade:
    1. Tesseract (fast, good for clean text) - First attempt
    2. PaddleOCR (accurate, excellent for light/faint text) - Second attempt
    3. EasyOCR (best for complex fonts/handwriting) - Final attempt
    """
    ...
    def __init__(self, api_key: str):
        ...
        # OCR readers (lazy load - only initialize when needed)
    ...
    def _get_paddleocr_reader(self):
        """Lazy load PaddleOCR reader (only when needed)"""
        if self.paddleocr_reader is None:
            try:
                from paddleocr import PaddleOCR
                logger.info("📦 Initializing PaddleOCR (one-time setup, may take 30s)...")
                self.paddleocr_reader = PaddleOCR(use_angle_cls=True, lang='en', show_log=False)
                logger.info("✓ PaddleOCR initialized successfully")
            except Exception as e:
                logger.error(f"❌ Failed to initialize PaddleOCR: {e}")
                self.paddleocr_reader = False  # Mark as unavailable
        return self.paddleocr_reader if self.paddleocr_reader else None
    ...
    def _validate_certificate_id(self, cert_id: Optional[str], issuer_name: Optional[str]) -> Optional[str]:
        ...
        # HubSpot: 32-char hex
        if 'hubspot' in issuer_lower:
            if len(cert_id) == 32 and all(c in '0123456789abcdefABCDEF' for c in cert_id):
                return cert_id.lower()
            else:
                logger.warning(f"Invalid HubSpot certificate ID format: '{cert_id}'")
                return None
    ...
    def _has_critical_data(self, text: str) -> bool:
        ...
        has_org_indicator = any(word in text_lower for word in 
                               ['udemy', 'coursera', 'hubspot', 'google', 'ibm', 
                                'microsoft', 'edx', 'linkedin', 'academy'])
        ...
        if is_complete:
            logger.info(f"✓ OCR appears complete ({word_count} words, indicators found)")
        else:
            logger.info(f"⚠️ OCR may be incomplete (words: {word_count}, cert: {has_name_indicator}, org: {has_org_indicator}, id: {has_id_pattern})")
        
        return is_complete

    def _merge_ocr_results(self, text1: str, text2: str, source_name: str) -> str:
        ...
        if len(unique_words2) > 5:
            logger.info(f"✓ Merging: {len(unique_words2)} unique words from {source_name}")
            return text1 + "\n" + text2
        else:
            logger.info(f"✓ Minimal new content from {source_name}, keeping primary text")
            return text1
        ...
        def _ocr_task():
            ...
            if raw_text:
                ...
                if corner_text and len(corner_text.strip()) > 5:
                    logger.info(f"✓ Tesseract corner OCR found additional text")
                    raw_text = raw_text + "\n" + corner_text
    ...
    async def _perform_paddleocr(self, image_bytes: bytes) -> Tuple[str, float]:
        """
        Perform PaddleOCR
        Returns: (text, processing_time)
        """
        ...
        try:
            ...
            if combined_text and len(combined_text.strip()) > MIN_OCR_TEXT_LENGTH:
                logger.info(f"✓ PaddleOCR: {elapsed:.2f}s, {len(combined_text)} chars")
            else:
                logger.warning(f"⚠️ PaddleOCR: Low extraction ({len(combined_text) if combined_text else 0} chars)")
            
            return combined_text, elapsed
        except asyncio.TimeoutError:
            raise TimeoutError(f"PaddleOCR exceeded timeout of {OCR_TIMEOUT_SECONDS}s")
    ...
    async def _perform_ocr_cascade(self, image_bytes: bytes) -> str:
        """
        Perform OCR with smart cascade: Tesseract → PaddleOCR → EasyOCR
        ...
        # STEP 2: If incomplete, try PaddleOCR
        if not has_enough_data:
            logger.info("📦 Step 2: PaddleOCR (fallback for better accuracy)...")
            paddleocr_text, paddleocr_time = await self._perform_paddleocr(image_bytes)
            total_time += paddleocr_time
            
            if paddleocr_text:
                final_text = self._merge_ocr_results(tesseract_text, paddleocr_text, "PaddleOCR")
                
                # Check again if we now have enough data
                has_enough_data = self._has_critical_data(final_text)
                
                if has_enough_data:
                    logger.info(f"✓ PaddleOCR filled gaps! Total time: {total_time:.2f}s")
                    return final_text
            else:
                logger.warning("⚠️ PaddleOCR didn't produce additional text")
        else:
            logger.info(f"✓ Tesseract complete, skipping PaddleOCR and EasyOCR ({tesseract_time:.2f}s)")
            return final_text
        
        # STEP 3: If still incomplete, try EasyOCR (final attempt)
        if not has_enough_data:
            logger.info("📚 Step 3: EasyOCR (final attempt for complex text)...")
            easyocr_text, easyocr_time = await self._perform_easyocr(image_bytes)
            total_time += easyocr_time
            
            if easyocr_text:
                final_text = self._merge_ocr_results(final_text, easyocr_text, "EasyOCR")
                logger.info(f"✓ Used all 3 OCR engines. Total time: {total_time:.2f}s")
            else:
                logger.warning("⚠️ EasyOCR didn't produce additional text")
        
        logger.info(f"✓ OCR cascade complete. Total time: {total_time:.2f}s")
        return final_text

    async def extract(self, image_bytes: bytes) -> dict:
        """
        Extract certificate data using triple OCR cascade + Mistral LLM
        
        Flow:
        1. Validate image
        2. Try Tesseract OCR (fast, 1-2s)
        3. If incomplete → Try PaddleOCR (accurate, 2-4s)
        4. If still incomplete → Try EasyOCR (complex, 3-5s)
        5. Merge OCR results
        6. QR code detection
        ...
        try:
            ...
            # STEP 1-3: OCR with smart cascade
            ...
            # STEP 5: LLM extraction with Mistral
            logger.debug("🤖 Step 4: Mistral LLM extraction...")
            prompt = f"""
...
3. issuer_name: The platform/organization name ONLY (e.g., "Coursera", "Udemy", "edX", "Google", "LinkedIn", "HubSpot Academy")
4. issuer_url: The full verification URL if present

CRITICAL for certificate_id - Platform-Specific Formats:
- **Udemy**: Starts with "UC-" followed by UUID (e.g., "UC-9ba43c6a-3983-495c-beb2-329801af4557")
- **Coursera**: Alphanumeric string (e.g., "ALS76DHQNMVZ")
- **HubSpot**: 32-character hex string (e.g., "93006c20260f4c788fc6c73a73503b84")
  * Fix OCR errors: é→6, ö→o, ï→i, ê→e
- **edX**: Long hex string or UUID
- IGNORE short reference numbers like "0004"
